...
public class MainActivity extends AppCompatActivity {
    ...
    public void tanimla() {
        username = getIntent().getExtras().getString("kadi");//bundle ilede alabilirdik
        firebaseDatabase = FirebaseDatabase.getInstance();
        reference = firebaseDatabase.getReference();
        list = new ArrayList<>();

        userRecyView = findViewById(R.id.userRecyView);
        userRecyView.setLayoutManager(new GridLayoutManager(MainActivity.this, 2));
    ...
    public void listele() {
        reference.child("Kullanıcılar").addChildEventListener(new ChildEventListener() {
            ...
            public void onChildAdded(@NonNull DataSnapshot dataSnapshot, @Nullable String s) {
                if (!dataSnapshot.getKey().equals(username)) { //kullanicinin kendi isminin listelenmesi mantıksız ondan kullandık
